...
;;;    require a structured representation. For classes, methods
;;;    should specialize `xml->' on the actual class, not its
;;;    name. This allows instances as the second argument of
;;;    `xml->'. The creation of instances can be handled automatically
;;;    (see remark below) so that just specifying a class name as TYPE
;;;    still works.
;;;
;;;    Obviously, this cannot be done if the actual class is
;;;    determined by means of inspecting VALUE. In such cases, the
;;;    name of a common superclass or some other distinctive symbol
;;;    should be used as specializer.
;;;
;;; In addition, there is the following rule: If TYPE is a symbol
;;; other than 'string, 'list or any other type that is either
;;; `read'able or specializes `xml->', TYPE is taken to designate a
;;; class. In that case, `make-instance' is called with TYPE and
;;; `xml->' is called with the resulting instance as TYPE.
...
;;;    is implemented. Otherwise, dispatching on VALUE is sufficient
